...
(defn convert-to-utilmd-xml
  ...
  (let [data (first (:messages data))]
    (xml/sexp-as-element
     [:UTILMD {}
      [:UtilmdHeader {}
       [:IGVersion {:S009_0065 (:message-type (:message data))
       ...
       [:Transactions {}
        (for [transaction (:transactions data)]
          [:Transaction {}
           ...
           [:BalanceResponsible {:T3035 (:org (:balance-responsible transaction))
                                 :C082_3055 (:type-code (:balance-responsible transaction))}
            (:id (:balance-responsible transaction))]
           [:BalanceSupplier {:T3035 (:org (:balance-supplier transaction))
                              :C082_3055 (:type-code (:balance-supplier transaction))}
            (:id (:balance-supplier transaction))]
           [:Consumer {:T3035 (:type (:consumer transaction))}
            [:PartyName {} (:name (:consumer transaction))]
            [:CityName {}]
            [:ZipCode {}]
            [:CountryCode {}]]
           [:MeterLocationAddress {:T3035 (:type (:metering-point transaction))}
            [:StreetName {} (:street-name1 (:metering-point transaction))]
            [:StreetName2 {} (:street-name2 (:metering-point transaction))]
            [:HouseNumber {} (:house-number (:metering-point transaction))]
            [:RoadCode {}
             [:CountryCode {}]
             [:StreetCode {}]
             [:HouseNumber {}]
